...
export default function WallPage() {
      ...
      try {
        ...
        const { data: postsData, error } = await supabase
          ...
          .select(`
            ...
            likes ( user_id ),
            comments ( id, content, created_at, profiles ( id, display_name, avatar_url, role ) )
          `)
        ...
        if (error) {
          ...
          const formattedPosts = (postsData as unknown as Post[]).map(post => ({
            ...post,
            comments: (post.comments ||[]).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
          }));
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (postMedia) {
        ...
        if (!uploadError) {
          uploadedMediaUrl = supabase.storage.from('post_media').getPublicUrl(fileName).data.publicUrl;
        }
      }
  ...
  return (
    <main className="min-h-screen flex flex-col items-center pt-32 pb-32 px-4 sm:px-6 relative">
                ...
                <div key={post.id} className="border border-zinc-900 bg-black p-4 sm:p-6 group flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    {currentUser?.id === post.profiles?.id && (
                      <button onClick={() => handleDeletePost(post.id)} className="text-zinc-600 hover:text-red-500 transition-colors p-2">
                        <Trash2 size={16} />
                      </button>
                    )}
                  </div>

                  {post.content && <p className="text-sm font-inter text-zinc-300 leading-relaxed mb-6 whitespace-pre-wrap">{post.content}</p>}
                      ...
                      <form onSubmit={(e) => handleAddComment(post.id, e)} className="flex items-end gap-3 mt-2">
                        <div className="relative w-full">
                          <input
                            ...
                            className="w-full bg-zinc-950 border border-zinc-900 py-2.5 px-4 pr-10 text-xs font-inter text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-700 transition-colors"
                          />
                          <button type="submit" disabled={!commentTexts[post.id]?.trim()} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-white transition-colors disabled:opacity-50">
                            <Send size={14} />
                          </button>
                        </div>
                      </form>
      ...
      {canPost && (
        <button onClick={() => setIsModalOpen(true)} className="fixed bottom-28 right-6 md:right-12 w-14 h-14 bg-white text-black rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)] z-40">
            ...
            <form onSubmit={handleCreatePost} className="flex flex-col gap-4">
              ...
              <div className="flex justify-between items-center border-t border-zinc-900 pt-4 mt-2">
                <div className="flex gap-4">
                  ...
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2">
                    <ImageIcon size={20} /><span className="text-[10px] uppercase tracking-widest font-inter hidden sm:inline">Фото</span>
                  </button>
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2">
                    <Video size={20} /><span className="text-[10px] uppercase tracking-widest font-inter hidden sm:inline">Видео</span>
                  </button>
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-2">
                    <Music size={20} /><span className="text-[10px] uppercase tracking-widest font-inter hidden sm:inline">Аудио</span>
                  </button>
                </div>
