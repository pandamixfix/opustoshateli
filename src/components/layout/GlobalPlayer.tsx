        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 w-[60%] sm:w-1/3 cursor-pointer sm:cursor-default" onClick={() => { if (window.innerWidth < 640) setIsExpanded(true); }}>
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[10px] font-inter tracking-[0.2em] text-zinc-500 uppercase truncate">{currentTrack.artist}</span>
            </div>
          </div>
          ...
          <div className="hidden sm:flex items-center justify-end gap-3 w-1/3">
            <div className="flex items-center gap-3 w-full max-w-xs mr-6">
              <span className="text-[10px] font-inter text-zinc-600 w-8 text-right">{formatTime(currentTime)}</span>
              <input type="range" min={0} max={duration || 100} value={currentTime} onChange={handleSeek} className="w-full h-1 bg-zinc-800 rounded-none appearance-none cursor-pointer accent-zinc-300" />
              <span className="text-[10px] font-inter text-zinc-600 w-8">{formatTime(duration)}</span>
            </div>
            <button onClick={toggleMute} className="text-zinc-500 hover:text-zinc-200 transition-colors">{isMuted ? <VolumeX size={16} /> : <Volume2 size={16} />}</button>
            <input type="range" min={0} max={1} step={0.01} value={isMuted ? 0 : volume} onChange={handleVolumeChange} className="w-20 h-1 bg-zinc-800 rounded-none appearance-none cursor-pointer accent-zinc-300" />
          </div>
        </div>
      </div>
          ...
          <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed top-0 left-0 w-full h-dvh z-100 bg-black flex flex-col sm:hidden">
            ...
            <div className="relative z-10 flex flex-col h-full px-6 pt-8 pb-[calc(2rem+env(safe-area-inset-bottom))] overflow-y-auto">
              ...
              <div className="flex flex-col items-center text-center mb-8 shrink-0">
                <h2 className="text-3xl font-playfair tracking-widest uppercase text-white mb-2 line-clamp-1">{currentTrack.title}</h2>
                <p className="text-xs font-inter tracking-[0.3em] uppercase text-zinc-400">{currentTrack.artist}</p>
              </div>

              <div className="flex flex-col gap-2 mb-8 w-full max-w-sm mx-auto shrink-0">
                <input type="range" min={0} max={duration || 100} value={currentTime} onChange={handleSeek} className="w-full h-1 bg-zinc-800 rounded-none appearance-none cursor-pointer accent-white" />
