...
interface Socials { tg?: string; vk?: string; inst?: string; }
interface MemberCardProps { name: string; role: string; description: string; imageUrl: string; socials: Socials; priority?: boolean; }
...
export default function MemberCard({ name, role, description, imageUrl, socials, priority = false }: MemberCardProps) {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-2xl font-playfair tracking-widest text-zinc-100 mb-2 uppercase text-center">{name}</h3>
      <p className="text-[10px] font-inter tracking-[0.3em] text-zinc-500 uppercase mb-6 text-center">{role}</p>
      <p className="text-sm font-inter text-zinc-400 text-center leading-relaxed mb-8 grow">{description}</p>

      <div className="flex gap-6 mt-auto">
        {socials.tg && <Link href={socials.tg} target="_blank" className="text-xs font-inter tracking-[0.2em] text-zinc-600 hover:text-zinc-200 transition-colors uppercase">Telegram</Link>}
        {socials.vk && <Link href={socials.vk} target="_blank" className="text-xs font-inter tracking-[0.2em] text-zinc-600 hover:text-zinc-200 transition-colors uppercase">VK</Link>}
      </div>
    </div>
  );
}